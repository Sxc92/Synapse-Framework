...
一个基于 Spring Boot 3.x 的企业级微服务框架，专注于简化开发流程、提高代码质量和系统性能。提供完整的认证授权、数据访问、缓存、事件驱动等企业级功能。
...
- ✅ **异常处理** - 统一的异常处理机制，支持国际化错误消息
- ✅ **智能分页** - 自动分页和结果封装，支持聚合查询
- ✅ **认证授权** - 基于 Token 的认证，支持滑动过期、自动续期
...
| 模块 | 描述 | 主要功能 | 文档 |
|------|------|----------|------|
| **synapse-core** | 核心模块 | 统一响应（Result）、异常处理（Ex）、工具类、国际化支持 | [📖 文档](synapse-core/README.md) |
| **synapse-databases** | 数据库模块 | BaseRepository、动态数据源、VO 映射、查询构建器、审计字段 | [📖 文档](synapse-databases/README.md) |
| **synapse-security** | 安全模块 | Token 认证、权限控制、滑动过期、自动续期、Gateway 签名验证 | [📖 文档](synapse-security/README.md) |
| **synapse-cache** | 缓存模块 | 二级缓存（Caffeine + Redis）、分布式锁、用户会话管理、缓存预热 | [📖 文档](synapse-cache/README.md) |
| **synapse-events** | 事件模块 | 事件驱动、同步/异步事件、事件发布订阅 | [📖 文档](docs/modules/synapse-events/README.md) |
| **synapse-i18n** | 国际化模块 | 多语言支持、错误消息国际化、消息资源管理 | [📖 文档](synapse-i18n/README.md) |
